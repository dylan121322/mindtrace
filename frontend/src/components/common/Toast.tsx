...
export function emitToast(kind: ToastKind, message: string) {
  BUS.dispatchEvent(new CustomEvent('mindtrace-toast', { detail: { kind, message } }));
}
...
export const ToastBridge: React.FC = () => {
  ...
  useEffect(() => {
    ...
    BUS.addEventListener('mindtrace-toast', handler);
    return () => BUS.removeEventListener('mindtrace-toast', handler);
  }, [toast]);
  return null;
};
