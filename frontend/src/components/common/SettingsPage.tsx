...
export const MEMBER_RANK_LIMIT_KEY = 'mindtrace_member_rank_limit';
export const MEMBER_NAME_WIDTH_KEY = 'mindtrace_member_name_width';
...
const TTS_PRESETS: TtsPreset[] = [
  ...
  {
    label: '自建 Edge-TTS',
    // 默认 host.docker.internal：MindTrace backend 跑在容器里时，localhost 指容器自身
  ...
  const switchToProfile = async (id: string) => {
    ...
    try {
      ...
      if (r.error) {
        ...
        localStorage.removeItem('mindtrace_hasStarted');
      ...
      <section className="mb-8" data-settings-tags="播客 podcast tts 语音 朗读 openai voice 主持人 硅基流动 siliconflow edge-tts">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 dk-card dk-border space-y-3">
          ...
          {ttsConfig.base_url && /:5050(\/|$)/.test(ttsConfig.base_url) && (
            <div className="rounded-xl border border-blue-100 dark:border-blue-500/30 bg-blue-50/70 dark:bg-blue-500/10 p-3 space-y-2">
              ...
              <p className="text-[11px] text-blue-800/80 dark:text-blue-300/80 leading-relaxed">
                ...
                <strong>MindTrace 跑在 docker 里时</strong>请用预设里的 <code className="bg-white/60 dark:bg-black/30 px-1 rounded text-[10px]">http://host.docker.internal:5050/v1</code>（容器内 localhost 不等于宿主机）。声音字段支持所有 <code className="bg-white/60 dark:bg-black/30 px-1 rounded text-[10px]">zh-CN-*Neural</code> 等微软 Neural 声音名。完整声音列表见{' '}
                <button
                  type="button"
          ...
          <p className="text-[11px] text-gray-400 leading-relaxed">
            ...
            <a href="https://mindtrace.click/podcast.html" target="_blank" rel="noreferrer" className="text-[#07c160] hover:underline">播客文档</a>。
          </p>
        </div>
      </section>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              <a href="https://github.com/runzhliu/mindtrace/releases" target="_blank" rel="noreferrer" className="hover:text-[#07c160] underline">更新日志</a>
              {' · '}
              <a href="https://mindtrace.click" target="_blank" rel="noreferrer" className="hover:text-[#07c160] underline">官方文档</a>
              {' · '}
              <a href="https://github.com/runzhliu/MindTrace" target="_blank" rel="noreferrer" className="hover:text-[#07c160] underline">GitHub</a>
            </p>
          </div>
