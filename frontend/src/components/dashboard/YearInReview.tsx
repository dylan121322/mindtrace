...
export const YearInReview: React.FC<Props> = ({ contacts, globalStats, onClose }) => {
    ...
    result.push({
      ...
      content: (
        <div className="flex flex-col items-center justify-center h-full gap-6 text-white">
          ...
          <div className="text-sm opacity-70">MindTrace · 微信聊天数据分析</div>
        </div>
      ),
    });
    ...
    result.push({
      ...
      content: (
        <div className="flex flex-col items-center justify-center h-full gap-4 text-white">
          ...
          <div className="mt-4 text-xs opacity-50">MindTrace · mindtrace.click</div>
        </div>
      ),
    });
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      ...
      ctx.fillText('MindTrace · 微信聊天数据分析', (W/2)*S, (curY + 120)*S);
      curY += 140;
      ...
      ctx.font = `${10*S}px ${FONT}`;
      ctx.fillText('MindTrace · mindtrace.click', (W/2)*S, (curY + 45)*S);

      // 生成 QR 码
      const doSave = async (canvas: HTMLCanvasElement) => {
        const dataUrl = canvas.toDataURL('image/png');
        const path = await downloadPng(dataUrl, `mindtrace-${selectedYear}-review.png`);
      ...
      try {
        const qrDataUrl = await QRCode.toDataURL('https://mindtrace.click', { width: 64, margin: 1, color: { dark: '#ffffff', light: '#00000000' } });
