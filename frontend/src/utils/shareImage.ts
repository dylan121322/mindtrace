...
export async function generateReportImage(options: ReportImageOptions): Promise<string> {
  ...
  const [qrDataUrl, faviconImg, githubImg, avatarDataUrl] = await Promise.all([
    QRCode.toDataURL('https://mindtrace.click', { width: 96, margin: 1, color: { dark: '#1d1d1f', light: '#f8f9fb' } }),
  ...
  ctx.fillText('MindTrace', TX, 32 * S);
  ...
  ctx.font = `${11*S}px ${FF}`;
  ctx.fillText('https://github.com/runzhliu/mindtrace', (githubImg ? 52 : 36)*S, fY + 25*S);
  ...
  const filename = `mindtrace-social-report-${Date.now()}.png`;
...
export async function generateShareImage(options: ShareImageOptions): Promise<string> {
  ...
  // 文字组高 44px → top=(84-44)/2=20px；MindTrace 中心 y=32px；副标题中心 y=55px
  const TX = (faviconImg ? 90 : 36) * S;
  hCtx.textBaseline = 'middle';
  hCtx.fillStyle = '#ffffff';
  hCtx.font = `900 ${20*S}px ${FF}`;
  hCtx.fillText('MindTrace', TX, 32 * S);
  ...
  fCtx.fillText('https://github.com/runzhliu/mindtrace', (githubImg ? 52 : 36)*S, 25*S);
  ...
  try {
    ...
    const safeName = contactName
      ...
      : 'analysis';
    const filename = `mindtrace-${safeName}-${Date.now()}.png`;
...
export async function generateCloneChatImage(options: CloneChatImageOptions): Promise<string> {
  ...
  ctx.font = `900 ${20*S}px ${FF}`; ctx.fillText('MindTrace', TX, 32 * S);
  ctx.fillStyle = 'rgba(255,255,255,0.78)';
  ...
  const filename = `mindtrace-clone-${safeName}-${Date.now()}.png`;
  return downloadPng(dataUrl, filename);
}
